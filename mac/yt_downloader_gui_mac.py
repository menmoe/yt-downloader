...
Build into a standalone app with: ./build.sh
"""
...
SUCCESS   = "#22c55e"
WARNING   = "#f59e0b"
FONT      = ("SF Pro Display", 13) if sys.platform == "darwin" else ("Segoe UI", 11)
FONT_SM   = ("SF Pro Display", 11) if sys.platform == "darwin" else ("Segoe UI", 9)
FONT_LG   = ("SF Pro Display", 22, "bold") if sys.platform == "darwin" else ("Segoe UI", 18, "bold")
...
def resolve_path(raw: str) -> str:
    ...
    for k, v in shorthands.items():
        if low == k or low.startswith(k + "/") or low.startswith(k + os.sep):
            raw = str(v) + raw[len(k):]
            break
...
def build_ydl_opts(fmt, quality, outtmpl, progress_hook, log_hook):
    ...
    if fmt == "mp3":
        ...
        qmap = {
            ...
            "480p":  "bestvideo[vcodec^=avc1][height<=480]+bestaudio[acodec^=mp4a]/bestvideo[height<=480]+bestaudio/best",
        }
        common.update({
...
class RoundedButton(tk.Canvas):
    def __init__(self, parent, text, command, bg=RED, fg=TEXT,
        ...
        super().__init__(parent, bg=parent["bg"] if hasattr(parent, "__getitem__") else BG,
...
class App(tk.Tk):
    def __init__(self):
        ...
        self._selected    = []          # user-chosen subset
        self._downloading = False
        self._queue       = queue.Queue()
        self._fmt         = tk.StringVar(value="mp3")
        self._quality     = tk.StringVar(value="320")
        self._dest        = tk.StringVar(value=str(Path.home() / "Downloads" / "YT_Downloads"))

        self._build_ui()
        self.after(100, self._poll_queue)

        # Centre window after it's drawn
    ...
    def _build_ui(self):
        ...
        tk.Label(hdr, text="▶", font=("Arial", 22), bg=BG, fg=RED).pack(side="left")
        ...
        self._style_entry_border(url_row, url_entry)

        self._fetch_btn = RoundedButton(url_row, "  Fetch  ", self._fetch,
        ...
        self._list_header = tk.Label(list_frame,
        ...
        self._canvas  = tk.Canvas(self._check_outer, bg=SURFACE, highlightthickness=0,
                                   height=0)
        ...
        dest_row = tk.Frame(self, bg=BG)
        dest_row.pack(fill="x", **pad, pady=(6, 0))

        dest_entry = tk.Entry(dest_row, textvariable=self._dest, font=FONT_SM,
                              bg=SURFACE2, fg=TEXT_MID, insertbackground=TEXT,
                              relief="flat", bd=0)
        dest_entry.pack(side="left", fill="x", expand=True, ipady=8, ipadx=10)
        self._style_entry_border(dest_row, dest_entry)

        browse_btn = RoundedButton(dest_row, " Browse ", self._browse,
        ...
        prog_frame.pack(fill="x", **pad, pady=(0, 0))
    ...
    def _section_label(self, text, parent=None):
        p = parent or self
        tk.Label(p, text=text.upper(), font=(FONT[0], 9, "bold"),
                 bg=p["bg"], fg=TEXT_DIM, anchor="w").pack(
                     fill="x", padx=(0 if parent else 28), pady=(14, 0))

    def _style_entry_border(self, frame, entry):
        """Wrap an entry in a 1px border frame."""
        frame.configure(bg=BORDER, padx=1, pady=1)

    def _radio(self, parent, label, var, value, cmd=None):
        row = tk.Frame(parent, bg=BG)
        row.pack(anchor="w", pady=2)
        rb = tk.Radiobutton(row, text=label, variable=var, value=value,
                             font=FONT, bg=BG, fg=TEXT_MID,
                             selectcolor=BG, activebackground=BG,
                             activeforeground=TEXT, indicatoron=True,
                             command=cmd)
        rb.pack(side="left")
    ...
    def _style_progressbar(self):
        ...
        style.configure("TProgressbar", troughcolor=SURFACE2,
                         ...
                         darkcolor=RED_DARK, thickness=6)
    ...
    def _populate_checklist(self, entries):
        ...
        # Show checklist only for playlists
        visible = min(n, 8)
        row_h   = 30
        if n > 1:
    ...
    def _start_download(self):
        ...
        threading.Thread(target=self._download_thread, args=args, daemon=True).start()
            ...
            def make_hook(idx=i, tot=total):
                def hook(d):
                    if d["status"] == "downloading":
                        pct_s = d.get("_percent_str", "").strip().replace("%", "")
                        try:
                            pct = float(pct_s)
                        except ValueError:
                            raw = d.get("eta")
                            pct = 0
                        # combine per-video pct with overall progress
            ...
            while True:
                ...
                if kind == "entries":
                    ...
                    if messagebox.askyesno("Done!", f"Download complete!\n\nOpen folder?"):
                        os.system(f'open "{out_dir}"' if sys.platform == "darwin"
                                  else f'explorer "{out_dir}"')
